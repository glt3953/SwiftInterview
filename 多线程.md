...
## 十、设计题（场景落地）
图片上传，必须按顺序上传 A → B → C，且都在后台线程。
1. 使用 GCD 的自定义串行队列 + async：
```
...
2. 使用 DispatchSemaphore 来控制多个异步任务的执行顺序
信号量（Semaphore）是一种用于控制对共享资源访问的同步机制。在 GCD 中， DispatchSemaphore 可以用来：
    1. 限制同时执行的任务数量
    2. 控制任务的执行顺序
    3. 等待异步操作完成
```
// 创建一个串行队列用于上传任务
    private let uploadQueue = DispatchQueue(label: "com.app.imageUploadQueue", qos: .utility)
    // 创建一个信号量，初始值为1
    ...
    func uploadImages(images: [UIImage], completion: @escaping (Bool) -> Void) {
        ...
        uploadQueue.async { [weak self] in
            ...
            for (index, image) in images.enumerated() {
                ...
                // 模拟网络请求上传图片（异步操作）
                self.uploadImage(image, index: index) { success in
                    if !success {
                        allSuccess = false
                    }
                    
                    print("图片 \(index) 上传\(success ? "成功" : "失败")")
...
3. 使用 Operation 和依赖关系 ：创建 Operation 对象并设置依赖关系，更加面向对象。
```
let queue = OperationQueue()
let uploadA = BlockOperation { /* 上传图片A */ }
let uploadB = BlockOperation { /* 上传图片B */ }
let uploadC = BlockOperation { /* 上传图片C */ }

uploadB.addDependency(uploadA)
uploadC.addDependency(uploadB)

queue.addOperations([uploadA, uploadB, uploadC], waitUntilFinished: false)
```
4. 使用 async/await (iOS 15+)
```
func uploadImagesInOrder() async throws {
    try await uploadImage(imageA)
    try await uploadImage(imageB)
    try await uploadImage(imageC)
} 
```
顺序执行 ：虽然 uploadImage 是异步操作，但 await 关键字会暂停当前函数的执行，直到异步操作完成
